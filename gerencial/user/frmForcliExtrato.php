...
//WHERE DO MOVIMENTOCC
$whereMovCC = "WHERE movimentocc.ControleOrigem = 0 AND movimentocc.idEmpresa = $idEmpresa";
$paramsMovCC = [];
...
$paramsFinal = array_merge($params, $paramsMovCC);

/* ================= LISTA ================= */

// ANTES DE PEGAR REGISTROS AVULSOS DO MOVIMENTOCC
// $lista = ExSqlNET("
//     SELECT 
//         m.*,
//         f.Nome,
//         movimentocc.ControleOrigem,
...
$lista = ExSqlNET("
    ...
    FROM movimentocc
    LEFT JOIN forcli f ON f.id = movimentocc.idForcli
    $whereMovCC
    ...
    <div class="content">
        ...
        <form method="GET" id = "formFiltro" class="filters">
            ...
            <select name="status" id="status">
                ...
                <option value="2" <?=($statusFiltro=='2')?'selected':''?>>Or&ccedil;amento</option>
                <option value="1" <?=($statusFiltro=='1')?'selected':''?>>Pago</option>
                <option value="3" <?=($statusFiltro=='3')?'selected':''?>>D&eacute;bito</option>
                <option value="4" <?=($statusFiltro=='4')?'selected':''?>>D&eacute;bito pago</option>
            </select>
        ...
        <div class="table-wrapper">
            <table>
                ...
                <tbody>
                    ...
                    <!--<tr>-->
                    <!--<tr onclick="abrirLancamento(<?= $item['id'] ?>)" style="cursor:pointer;">-->
                    
                        <tr 
                            <?php if (!empty($item['id'])): ?>
                            onclick="abrirLancamento(<?= (int)$item['id'] ?>)"
                        ...
                        <td>
                            <?php if ($item['Status'] == 3): ?>
                                <span class="status pendente"><?= mb_convert_encoding('D�bito em aberto', 'UTF-8', 'ISO-8859-1') ?></span>
                        
                            <?php elseif ($item['Status'] == 4): ?>
                                <span class="status pago"><?= mb_convert_encoding('D�bito Pago', 'UTF-8', 'ISO-8859-1') ?></span>
